...
class AskTeacherController extends ApiController
{
    use SiteMeta ;
    ...
    public function goAsk(Request $request){
        ...
        if ( $data ) {
            return response()->json(["message" => "Question has been sent successfully !"]);
        }else{
            return response()->json("Error happened !",400);
        }
    }
    ...
    public function answer($id ,Request $request){
        ...
        if ( $answer ) {
            return response()->json(['message' => 'تم ارسال الجواب بنجاح']);
        }else{
            return response()->json("حدث خطأ", 400);
        }
    }
