            ...
            switch($processor){
                ...
                case "paypal" :
                    ...
                    if(!$buyPackage && $typ != 'course' ){
                        ...
                        $request->session()->flash('toast-success', "يتم تحويلك الى الشراء الان ");
                        //Redirect to paypal
                        $request->session()->flash('redirect_url', $buyPackage );
                        $request->session()->flash('redirect_after', 6000);
                    }
                        $request->session()->flash('toast-time', 6000);
                break;
                ...
                case "free":
                    ...
                    if(!$result && $typ != 'course'){
                        ...
                        $request->session()->flash('toast-success', "عفوا لقد قمت بشراء هذه الدورة المجانية من قبل لا يمكن شرائها مرة أخرى");
                    }else{
                        $request->session()->flash('toast-success', "تم الشراء بنجاح شكرا لك");
                        return redirect('/cp');
                    }
