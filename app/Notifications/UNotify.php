...
class UNotify extends Notification
{
    use Queueable;
     ...
     * @return void
     */
    public function __construct($data , $type , $lang = 'en' , $customMessage = '' , $customTitle = '' , $via = '') {
        $this->data = $data ;
        $this->type = $type ;
        $this->lang = $lang ;
        $this->message = html_entity_decode($customMessage) ;
    ...
    protected function getVia(){
        $via = Options::where('option_var' , $this->type)->where('type' , 'notifications_via')->select('option_value')->first();
    ...
    protected function getTitle(){
        ...
        if( isset($this->title[3]) ){
            return $this->title ;
        }

        $template = 'notifications.title.'.$this->type ;
    ...
    public function toMail($notifiable) {
        ...
        if( $this->message != '' ){
             $save = file_put_contents( base_path('resources/views/notifications/email/customMessage.blade.php') , $this->message ) ;
