...
class StudentRepository extends BaseRepository
{
    ...
    public function saveExam($examId , $answers ){
        ...
        DB::table('exams_results')->where('exam_id' , $examId)->where('user_id' , auth()->user()->id)->delete() ;
        DB::table('exams_answers')->where('exam_id' , $examId)->where('student_id' , auth()->user()->id)->delete() ;
    ...
    public function getPercent($list , $questions = []){
        ...
        if( $return['times'] == 0 ){ $return['times'] = 1 ; }
