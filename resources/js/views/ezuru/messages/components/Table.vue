<template>
    <div id="MessagesTable">
                <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
                    <el-table-column align="center" label="ID" width="50">
                        <template slot-scope="scope">
                        <span>{{ scope.row.id }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column align="center" label="Unit">
                        <template slot-scope="scope">
                                <router-link :to="'/ezuru/units/edit/'+scope.row.unit.id"><span>{{ scope.row.unit.title }}</span></router-link>
                        </template>
                    </el-table-column>

                    <el-table-column align="center" label="Owner">
                        <template slot-scope="scope">
                                <router-link :to="'/users/user/edit/'+scope.row.owner.id"><span>{{ scope.row.owner.name }}</span></router-link>
                        </template>
                    </el-table-column>

                     <el-table-column align="center" label="Guest">
                        <template slot-scope="scope">
                                <router-link :to="'/users/user/edit/'+scope.row.guest.id"><span>{{ scope.row.guest.name }}</span></router-link>
                        </template>
                    </el-table-column>

                    <el-table-column align="center" label="Last Message">
                        <template slot-scope="scope">
                            <div v-if="scope.row.last.length > 0">
                                <el-tooltip :content="'Last Message Time : '+scope.row.last[0].created_at">
                                        <span> {{ scope.row.last[0].message }} </span>
                                </el-tooltip>
                            </div>
                        </template>
                    </el-table-column>
                    
                    
                    <el-table-column align="center" label="Count">
                        <template slot-scope="scope">
                                <span>{{ scope.row.chat_count }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column align="center" label="Date">
                        <template slot-scope="scope">
                                <span>{{ scope.row.created_at }}</span>
                        </template>
                    </el-table-column>

                    <el-table-column align="center" label="Delete">
                        <template slot-scope="scope">
                                <el-button-group>
                                    <el-tooltip content="View" placement="top" v-permission="['manage messages view chat']" >
                                            <router-link :to="'/messages/chat/'+scope.row.id">
                                            <el-button size="mini" type="primary" icon="el-icon-view"></el-button>
                                            </router-link>
                                    </el-tooltip>
                                    <el-tooltip content="Delete" placement="top" v-permission="['manage messages delete']">
                                        <el-button size="mini" @click="handleDelete( scope.row.id , scope.row.title )" type="danger" icon="el-icon-delete"></el-button>
                                    </el-tooltip>
                            </el-button-group>
                        </template>
                    </el-table-column>
                    </el-table>
    </div>
</template>

<script>
    import settings from '@/settings' ;
    import MessagesResource from '@/api/ezuru/messages' ;
    const Messages = new MessagesResource() ;

    const Vue = window.vue;

    import permission from '@/directive/permission/index.js' ;

    export default {
        directives : { permission } ,
        data(){
            return {
                dialogVisible : false
            }
        },
        props : ['list' , 'loading' , 'reload' , 'type'],
        methods : {
            reloadAgain() {
                this.$emit('reloadAgain') ;
            }, 
            handleView(scope, name) {
                let sc = scope.row ;
                this.sc = sc ;
                this.dialogVisible = true ;
            },
            handleClose(){
                this.dialogVisible = false;
            },
            handleDelete(id, name) {
                let o = this ;
                this.$confirm('This will permanently delete Message ' + id + '. Continue?', 'Warning', {
                    confirmButtonText: 'OK',
                    cancelButtonText: 'Cancel',
                    type: 'warning',
                }).then(() => {
                    Messages.destroy(id).then(response => {
                        this.$message({
                            type: 'success',
                            message: 'Delete completed',
                        });
                        o.reloadAgain();
                    }).catch(error => {
                    console.log(error);
                    });
                }).catch((e) => {
                    console.log(e);
                    this.$message({
                    type: 'info',
                    message: 'Delete canceled',
                    });
                });
            }
        },
        computed : {
            
        }
    }
</script>
