        ...
        <h1 class="title">  </h1>
            ...
            <el-container>
                <el-header v-show="!show_list">
                    <h3>
                     {{ tit }} 
                     <el-button type="success" class="el-left"  v-permissionx="['manage user search']"  @click="show_list = true"><i class="el-icon-list"></i> Show List {{ type }}</el-button> </h3>
                    <hr/>
                </el-header>
                <el-header v-show="show_list">
                    <h3>
                     Show List of Questions
                     <el-button type="success" class="el-left"  v-permissionx="['manage user search']"  @click="show_list = false"><i class="el-icon-edit"></i> Add New {{ type }}</el-button> 

                     <el-button type="warning" class="el-right"  v-permissionx="['manage user search']"  @click="search.visible = true"><i class="el-icon-zoom"></i> Search </el-button> 
                ...
                <el-main height="">
                    <el-row >
                        ...
                        <el-col :span="24" v-show="show_list">
                            ...
                            <el-dialog title="Search" :visible.sync="search.visible">
                                ...
                                <span slot="footer" class="dialog-footer">
                                    <el-button @click="QuestionSearch(false)">Cancel</el-button>
                                    <el-button type="primary" @click="QuestionSearch(true)">Search</el-button>
                                </span>
                            </el-dialog>
                            
                              <el-table @selection-change="handleSelectionChange" ref="dragTable" :data="taxData" style="width: 100%" v-loading="listLoading"  row-key="id" border fit highlight-current-row >
                                    <el-table-column type="selection" width="55"></el-table-column>
                                    <el-table-column prop="id" width="50" label="ID"></el-table-column>
                                    <el-table-column label="Title">
                                        <template slot-scope="scope">
                                               <span v-html="scope.row.title.replace( /<.*?>/g, '' )"></span>     
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="category">
                                        <template slot-scope="scope">
                                               <span v-if="scope.row.category && scope.row.category.name"> {{ scope.row.category.name }}</span>     
                                        </template>
                                    </el-table-column>

                                    <el-table-column label="Subject">
                                        <template slot-scope="scope">
                                               <span v-if="scope.row.subject && scope.row.subject.name"> {{ scope.row.subject.name }}</span>     
                                        </template>
                                    </el-table-column>
                                        ...
                                        <template slot-scope="scope">
                                            ...
                                            <el-tooltip   content="Edit" placement="top">
                                                <router-link :to="'/set-questions/questions/edit/'+scope.row.id">
                                                    <el-button size="mini" type="warning" ><i class="el-icon-edit"></i></el-button>
                                                </router-link>
                                            </el-tooltip>

                                            <!-- v-permission="['manage '+ptype+' status']" -->
                                            <el-tooltip   v-if="scope.row.status == 1" content="Disable" placement="top">
                                                <el-button  size="mini" type="info" @click="ActiveQuestion(scope.row , 0)" ><i class="el-icon-error"></i></el-button>
                                            </el-tooltip>

                                            <!-- v-permission="['manage '+ptype+' status']" -->
                                            <el-tooltip    v-else-if="scope.row.status == 0" content="Active" placement="top">
                                                <el-button  size="mini" type="success" @click="ActiveQuestion(scope.row , 1)" ><i class="el-icon-success"></i></el-button>
                                            </el-tooltip>

                                            <el-tooltip content="Delete" placement="top"  > <!-- v-permission="['manage '+ptype+' delete']" -->
                                                <el-button size="mini" type="danger" @click="DeleteQuestion(scope.row)" ><i class="el-icon-delete"></i></el-button>
                                            </el-tooltip>
                                ...
                                <pagination v-show="pagination.total>0" :total="pagination.total" :page.sync="pagination.current_page" :limit.sync="search.limit" @pagination="gotoPage" />


                                 <el-divider></el-divider> 

                                <el-select v-show="false" v-model="selectType" placeholder="Selection Type">
                                    <el-option
                                ...
                                <el-select v-model="bulkAction" placeholder="Select bulk action">
                                    <el-option
                                ...
                                <el-select v-show="bulkAction == 'attach_to_exam'" v-model="attachtoExams" multiple filterable>
                                        <el-option v-for="item in exams" :key="item.id" :value="item.id" :label="item.title" />      
                                </el-select>

                                <el-button v-show="questions_to_attach.length > 0" @click="sendRequest" type="primary">Save Request</el-button>

                                
                                
                             
                                <el-dialog title="Attach questions to exam"
                                        :visible.sync="bulkActionAttachToExam"
                                        width="30%"
                                        :before-close="bulkActionAttachToExamClose">
                                        <span>This is a message</span>
                                        <span slot="footer" class="dialog-footer">
                                            <el-button @click="bulkActionAttachToExam = false">Cancel</el-button>
                                            <el-button type="primary" @click="bulkActionAttachToExam = false">Confirm</el-button>
                                        </span>
                                </el-dialog>
...
<script>
    ...
    export default {
        ...
        components : { Tinymce, Tinymcesm  , Pagination } , 
        data() {
           return {
               "show_list" : true ,
        ...
        methods : {
            ...
            getList : function( page = 1 , search = false , message = false , limit = 10 ){
                ...
                let u = settings.apiUrl+'admin/questions/?type='+ this.type+'&page='+page+'&limit='+this.search.limit ;
