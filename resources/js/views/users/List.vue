<template> 
  <div class="app-container">
    <div class="filter-container">
      <el-input v-permission="['manage admin search']" v-model="query.keyword" :placeholder="$t('table.keyword')" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-select v-permission="['manage admin search']" v-model="query.role" :placeholder="$t('table.role')" clearable style="width: 90px" class="filter-item" @change="handleFilter">
        <el-option v-for="item in roles" :key="item" :label="item | uppercaseFirst" :value="item" />
      </el-select>
      <el-button  v-permission="['manage admin search']" v-waves class="filter-item" type="primary" icon="el-icon-search" @click="handleFilter">
        {{ $t('table.search') }}
      </el-button>
      <el-button class="filter-item"  v-permission="['manage admin add']" style="margin-left: 10px;" type="primary" icon="el-icon-plus" @click="handleCreate">
        {{ $t('table.add') }}
      </el-button>
      <el-button  v-permission="['manage admin export']" v-waves :loading="downloading" class="filter-item" type="primary" icon="el-icon-download" @click="handleDownload">
        {{ $t('table.export') }}
      </el-button>
    </div>
    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="Actions" width="350">
        <template slot-scope="scope">
          <router-link  :to="'/users/admin/edit/'+scope.row.id" v-if="!scope.row.roles.includes('admin')">
            <el-button type="primary" size="small" icon="el-icon-edit" v-permission="['manage admin edit']">
              Edit
            </el-button>
          </router-link>
          <el-button type="warning" size="small" icon="el-icon-edit" v-permission="['manage admin premission']" v-role="['admin','manager']" v-if="!scope.row.roles.includes('admin')" @click="handleEditPermissions(scope.row.id);">
            Permissions
          </el-button>
          <el-button type="danger" size="small" icon="el-icon-delete" v-permission="['manage admin delete']" v-if="!scope.row.roles.includes('admin')" @click="handleDelete(scope.row.id, scope.row.name);">
            Delete
          </el-button>
        </template>
      </el-table-column>
...
import permissionx from '@/directive/permission'; // Waves directive
...
export default {
  ...
  created() {
    this.resetNewUser();
    this.getList();
    if ( checkPermission(['manage premissions edit']) ) {
      this.getPermissions();
    }
